...
class TestRTAC(unittest.TestCase):
    ...
    def test_policy(self):
        ...
        rtac = RTAC.RTAC(env, alpha=0.2, gamma=1, lr_pol=0.0003, lr_val=0.003, buffer_size=100, batch_size=8,
        ...
        env = rtmdp.RTMDP(probeEnvironments.TwoActionsTwoStates(), 0)
        rtac = RTAC.RTAC(env, alpha=100, gamma=1, lr_pol=0.003, lr_val=0.03, buffer_size=200, batch_size=16,
                         hidden_size=256, num_hidden=2)
        rtac.train(training_time=5)
        avg = utilities.evaluate_policy(rtac.pol_network.act, env, trials=1000)
...
class TestSAC(unittest.TestCase):
    ...
    def test_value_function(self):
        ...
        env = probeEnvironments.ConstRewardEnv()
        sac = SAC.SAC(env, alpha=0.2, lr_val=0.01, replay_size=1, batch_size=1, hidden_size=256, num_hidden=2)
        sac.learn(iterations=1000)
        self.assertTrue(abs(1 - sac.value([0, 1]).item()) < TOLERANCE)

        env = probeEnvironments.PredictableRewardEnv()
        sac = SAC.SAC(env, alpha=0.2, lr_val=0.01, replay_size=1, batch_size=1, hidden_size=256, num_hidden=2)
        sac.learn(iterations=1000)
        self.assertTrue(abs(1.0 - sac.value([1, 1]).item()) < TOLERANCE)
        self.assertTrue(abs(-1.0 - sac.value([-1, 1]).item()) < TOLERANCE)
    ...
    def test_policy(self):
        ...
        # Check if optimal policy can be adopted
        env = probeEnvironments.TwoActionsTwoStates()
        sac = SAC.SAC(env, alpha=1, gamma=1, lr_pol=0.003, lr_val=0.01, replay_size=100, batch_size=8,
                      hidden_size=256, num_hidden=2)
        sac.learn(iterations=1000)
        avg = utilities.evaluate_policy(sac.policy.act, env, trials=1000, rtmdp_ob=False)
        self.assertTrue(abs(2 - avg) < TOLERANCE)
        self.assertTrue(abs(2 - sac.value([0, 1]).item()) < TOLERANCE)
        self.assertTrue(abs(2 - sac.value([1, 1]).item()) < TOLERANCE)
        self.assertTrue(abs(1 - sac.value([2, 1]).item()) < TOLERANCE)
        self.assertTrue(abs(1 - sac.value([3, 0]).item()) < TOLERANCE)

        # Check if random policy is adopted when entropy is valued extremely highly
        env = probeEnvironments.TwoActionsTwoStates()
        sac = SAC.SAC(env, alpha=10, gamma=1, lr_pol=0.003, lr_val=0.03, replay_size=200, batch_size=16,
                        hidden_size=256, num_hidden=2)
        sac.learn(iterations=1000)
        avg = utilities.evaluate_policy(sac.policy.act, env, trials=1000, rtmdp_ob=False)

        self.assertTrue(abs(1.5 - avg) < TOLERANCE)
        self.assertTrue(abs(2 * 10 * math.log(2, math.e) - sac.value([0, 1]).item()) < 10)
        self.assertTrue(abs(1 * 10 * math.log(2, math.e) - sac.value([3, 1]).item()) < 10)
