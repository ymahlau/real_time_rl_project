...
class TestRTAC(unittest.TestCase):
    ...
    def test_policy(self):
        ...
        rtac.train(num_episodes=2000)
        avg = evaluate_policy(rtac.network.policy_network.act, env, trials=1000)

        self.assertAlmostEqual(1.5, avg, delta=0.2)
        self.assertAlmostEqual(2 * 100 * math.log(2, math.e), rtac.get_value(([0], 0)).item(), delta=10)
        self.assertAlmostEqual(1 * 100 * math.log(2, math.e), rtac.get_value(([3], 0)).item(), delta=10)
