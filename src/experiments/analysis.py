...
def analyse_experiments(data_paths: List[str]) -> List[List[float]]:
    ...
    for data_path in data_paths:
        ...
        if num_data_points != len(data_single_exp):
            raise ValueError("Data format is invalid: The amount of data points in {0}.csv differs from {1}.csv".format(data_path, data_paths[0]))
        data.append(numpy.array(data_single_exp))
    ...
    for i in range(len(data[0])):
        ...
        mean = numpy.mean(avg_rewards)
        if numpy.std(avg_rewards) == 0:
            statistics.append([num_steps, mean, mean, mean])
        else:
            conf_interval = bootstrap(avg_rewards, numpy.mean, confidence_level=0.95).confidence_interval
